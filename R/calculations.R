...
#'
#' @param data Dataframe in wide format (i.e., one row for each HoNOS
#' assessment and one column for each HoNOS item)
#' @param id_var Name of variable that uniquely identifies each individual
#' @param date_var Name of date (or datetime) variable
#' @param add_change_label Logical, specifying whether to add a variable that
#' describes the change using the function \code{\link{calc_change_label}}
#' @param change_label String, specifying whether to describe the change using
#' "high_low" (e.g., HL stands for high to low) or "deterio_improve"
#' (Deterioration, Unchanged, Improved)
...
lag_honos <- function(data,
  ...
  data_lag <- honos_renamed %>%
    ...
    dplyr::mutate(dplyr::across(
      .cols = c({{ date_var }}, {{ honos_scales_new_names }}),
  ...
  data_change <- data_lag %>%
    tidyr::pivot_longer(
      cols = dplyr::all_of(c({{ honos_scales_new_names }},
    ...
    dplyr::mutate(
      item = as.numeric(stringr::str_extract(item, "\\d+")),
    ...
    dplyr::mutate(
      honos_change = lag1honos_value - honos_value,
      honos_date_diff := lubridate::as.difftime(
        {{ date_var }} - !!rlang::sym(paste0("lag1", rlang::ensym(date_var))))
    ) %>%
  ...
  if (add_change_label == TRUE) {
    data_change <- data_change %>%
      dplyr::mutate(honos_change_label = calc_change_label(
        value = honos_value,
...
#' @param lag_value Name of variable with HoNOS score at previous time point
#' (t-1)
#' @param change_label String, specifying whether to describe the change using
#' "high_low" (e.g., HL stands for high to low) or "deterio_improve"
#' (Deterioration, Unchanged, Improved)
...
#' )
calc_change_label <- function(value,
                              lag_value,
                              change_label = c("high_low",
                                               "deterio_improve")) {
  change_label <- match.arg(change_label)
  ...
  x4 <- dplyr::case_when(
    {{ value }} %in% c(3:4) & {{ lag_value }} %in% c(0:2) ~ "HL",
...
#' @param return_items Logical, specifying whether to include individual
#' item scores. NOTE: CURRENTLY NOT SUPPORTED.
#'
#' @return
#' @export
#'
...
calc_subscales <- function(data,
  ...
  data_temp <- data %>%
    dplyr::mutate(honos_subscale = dplyr::case_when(
      {{ item_var }} %in% c(1:3) ~ "behaviour",
