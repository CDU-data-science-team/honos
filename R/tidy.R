...
#'
#' @description This functions transforms HoNOS data from wide (i.e., one row
#' per assessment, one column per variable) to long (i.e., one row per item)
#' data. This data format is needed to perform common calculations and
#' visualisations of HoNOS data in R. This function automatically replaces the
#' value 9 with a missing value ('NA').
#'
...
#' @param pivot String, specifying whether to include item 8 problem and item
#' 8 specification in the long format ('all_items') or whether to only include
#' all item scores in long format ("item_scores").
#'
#' @return
#' @export
#'
...
pivot_honos_longer <- function(data,
  ...
  honos_new_var_names <- rename_honos(
    data = data,
    value_vars_current = value_vars_current,
    prob_var_item8 = prob_var_item8,
    spec_var_item8 = spec_var_item8,
  ...
  # nor make data long, first need to make sure all values are same type,
  # I dont really like this
  honos_long <- honos_renamed %>%
    dplyr::mutate_if(is.double, as.character) %>%
    tidyr::pivot_longer(
      cols = dplyr::all_of(honos_new_var_names),
      names_sep = "_",
  ...
  if (pivot == "all_items") {
    ...
    # this may be useful when I only want to have actual numeric values in the
    # values column
