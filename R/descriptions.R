...
#' @description Types of other mental and behavioural problems specified in
#' Item 8 can be labelled using the function \code{\link{as_i8_desc}}
#'
#' @param x Vector of item numbers
#' @param .ignore_n_items Logical, if TRUE the specified vector does not check
#' the item number and assumes that the 18 item "working adults" version that
#' includes current and historical scales.
#'
#' @return Labelled "factor"
#' @export
#'
...
as_item_desc <- function(x, .ignore_n_items = FALSE) {
  ...
  if (.ignore_n_items == FALSE) {
    ...
    # Check if max number of items is already implemented in this version of
    # the package
...
as_cluster_desc <- function(x,
  ...
  if (return == "cluster") {
    ...
    if (super_cluster_details == FALSE) {
      ...
      x <- factor(x,
        levels = c(
          "A: Non-psycotic (Blank place marker)",
